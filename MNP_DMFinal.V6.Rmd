...
# Etude de la densité g des X
On utilise les données Data1. Jeux de données $(X_i,Y_i)$ i=1,..,2000 
<br>
Avec la représentation suivante $Y_i = r(X_i) + \sigma\xi_i(X_i)$ où $(X_i,Y_i)$ sont i.i.d. et $\sigma$ ne dépend pas de X (homoscédastique).
<br>
- La loi de $X_i$ admet une densité g(x)
<br>
- Les $\xi_i$ sont i.i.d avec ${\rm I\!E}(\xi_i)=0$ et ${\rm I\!E}(\xi_i^2)=1$ et ont une densité $\mu$. 
<br>
- $X_i$ et $\xi_i$ sont indépendants.
...
Pour avoir une idée de la densité g de X on peut tracer son histogramme qui est un estimateur de la densité. Obtenu par la série d'approximation suivante:
<br>
$g(x_0) \displaystyle \approx_{h \to 0} \frac{{\rm I\!P} (X \in [x_0-h,x_0+h])}{2h} \approx_{n \to \infty} \frac{1}{2nh}\sum_{i=1}^{n} 1_{X_i \in[x_0-h,x_0+h]} := \hat{g}_{n,h}(x_0)$ 
...
## Estimateur par noyau de la densité g(x) de X
La méthode par noyau est une généralisation de la méthode d'estimation par histogramme.
Elle permet de construire un estimateur avec de meilleur propriété de régularité notamment de continuité.
<br>
On a un échantillon $(X_1,X_2,...X_n)$ i.i.d, réalisation d'une variable aléatoire X dont on cherche à estimer la densité g.
<br>
Un estimateur par noyaux est donné par $\hat{g}(x_0)= \frac{1}{nh}\sum_{i=1}^{n} K(\frac{x_0-X_i}{h})$
<br>
K est le noyau régularisant que l'on prend gaussien $K(x) = \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}x^2}$
<br>
L'estimateur est basé sur la série d'approximation suivante :
<br>
$g(x_0)\displaystyle \approx_{h \to 0}E(\frac{1}{h} K(\frac{x_0-X}{h} ))\approx_{n \to \infty} \frac{1}{nh}\sum_{i=1}^{n} K(\frac{x_0-X_i}{h}):= \hat{g}_{n,h}(x_0)$

Pour implémenter ces méthodes, dans le cas de l'estimation de la densité on va utiliser la fonction bkde du package KernSmooth. 
On prendra pour noyau le noyau normal.Ce choix peut sembler arbitraire, mais on a vu que ce n'est pas le choix du noyau qui est le plus important dans l'estimation de la densité.
...
```{r plot00_d1, echo=FALSE, fig.height=10, fig.width=8, warning=FALSE}
par(mfrow=c(3,1))
xlim <- c(-0.2,6)
ylim <- c(0,2)
...
```{r plot033_d1, echo=FALSE, fig.height=6, fig.width=10, warning=FALSE}
...
## Estimation par noyau gaussien de la densité g(x) de X avec un h optimal
On commence par déterminer un paramètre de lissage ou fenêtre h optimale. Dans le sens qu'elle minimise une fonction de côut, ou critère d'erreur.
Du type MSE (Erreur quadratique moyenne), MISE (Erreur Quadratique Intégrée Moyenne) ou AMISE (Erreur Quadratique Intégrée Moyenne Asymptotique).
<br>
Plus pécisémmment on introduit l'estimateur du risque par validation croisé 
<br>
$\hat{J}(h)={\displaystyle \int_{\rm I\!R} {\hat{f}_{n,h}(x)}^2\,dx}-\frac{1}{2n}\sum_{i=1}^{n}\hat{f}_{(-i),n,h}(X_i)$ 
<br>
Avec $\hat{f}_{(-i),n,h}$ obtenu en calculant l'estimateur sur l'échantillon privé de la donnée $X_i$.
<br>
On choisi $\hat{h}$ qui minimise ce risque $\hat{J}(\hat{h})=min_h \hat{J}(h)$
<br>
On obtient cette fenêtre optimale par la fonction bw.ucv de la librairie stats qui implémente la validation croisée, mais aussi en utilisant d'autres méthodes.    
  
### Fenêtre h optimale par validation croisée 

#### Utilisation de la fonction bw.ucv library stats
Cette fonction est adaptèe à la validation croisée en densité.   
```{r warning=FALSE, echo=FALSE}
  h_ucv<-bw.ucv(X)
  h_ucv
```
  
### Fenêtre h optimale par la règle de Silverman
...
```{r echo=FALSE, fig.height=5, fig.width=10}
  ...
  plot(estim_ucv$x,estim_ucv$y,type="l",col="black", lty=3, 
       main ="Estimateurs par noyaux gaussien de la densité g des X et différents h.", xlab="X", ylab="densité g estimée")
...
### Fenêtre h optimale - Méthodes alternatives
  
#### Fonction density
On peut regrarder le résultat de la fonction density du package RSmooth qui teste différents noyaux et renvoie un h optimal 
...
estim_d <- bkde(x=X,bandwidth=h_density,gridsize=size)
...
## Zone de l'espace où l'estimation de r sera plus précise
...
On utilise les données de Data1, (X,Y1).

## Linéarité de la fonction r
Pour vérifier la linéarité on va tracer sur un même graphique, le nuage de points ainsi que la droite de régression linéaire Y~X.
On va y ajouter la courbe de regression locale de type loess. Avec cette méthode l'ajustement se fait localement, par ajustement d'un polynôme de degès 1 ou 2. 
Cette dernière courbe indique la tendance globale entre nos 2 variables, que l'on va comparer à la droite de régression linéaire.

On utilise la fonction de R stat_smooth de la librairie ggplot2 avec l'option method=lm pour construire la régression linéaire simple (en rouge). 
et l'option method=loess pour construire la régression de type loess (en bleue). 

- Tout d'abord on trace Y1 en fonction de X.

```{r plot211_d1, echo=FALSE, fig.height=5, fig.width=10}
  qplot(X,Y) + 
  geom_point(colour="grey60") + 
  stat_smooth(method=loess,col="blue", fill="#56B4E9")+
  stat_smooth(method=lm, se=FALSE, colour="red") + 
  theme(legend.position="top") +
  xlab("X") +  ylab("Y1")+ geom_rug() + scale_color_brewer(palette="Paired") + theme_minimal() 

```
Sans transformation, à la vue du graphique, r ne peut être linéaire. 
La forme de la courbe loess suggère d'utilser la transformation log(X) pour linéariser. 

- Maintenant On trace Y1 en fonction de log(X)

```{r plot212_d1, echo=FALSE, fig.height=5, fig.width=10}
  qplot(log(X),Y) + 
  geom_point(colour="grey60") + 
  stat_smooth(method=loess,col="blue", fill="#56B4E9")+
  stat_smooth(method=lm, se=FALSE, colour="red") + 
...
Dans la région $X_i\in[-5,1]$ où l'on retouve la quasi totalité de l'échantillon, on a une bonne adéquation des courbes loess et lm, la transformtion (log(x)) a permis de bien linéariser.

<div style="page-break-after: always;"></div>

## Construction d'un estimateur non-parametrique de r(x) par noyaux régularisants
Le noyau permet de lisser le bruit $\xi_i$ et localise l'information au niveau d'un point, en y ajoutant un poids. Un peu comme une distribution ou masse de Dirac.
Ces méthodes de lissage par noyau consistent à effectuer au voisinage de chaque point une régression locale.
Qui dans le cas Nadaraya-Watson est linéaire et est polynomiale dans le cas des méthodes par polynômes locaux. 
La localisation est assurée par la fonction de poids le noyau K. 
On le prend ici gaussien $K(x) = \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}x^2}$
<br>
Plus précisémment en posant $K_h(x_0-x)=\frac{1}{h}K(\frac{x_0-x}{h})$ on a en utilisant la représentation $Y_i=r(X_i)+\sigma\xi_i$ que
$\frac{1}{n}\sum_{i=1}^{n}K_h(x_0-X_i)Y_i = \frac{1}{n}\sum_{i=1}^{n}K_h(x_0-X_i)r(X_i) + \frac{1}{n}\sum_{i=1}^{n}K_h(x_0-X_i)\sigma\xi_i \text{  (*)}$ 
$\text{(*)}\approx_{n \to \infty}{\rm I\!E}[K_h(x_0-X_i)r(X_i)] = \int_{\rm I\!R}K_h(x_0-x)r(x)g(x)dx)\approx_{h \to 0} r(x_0)g(x_0)$
<br>
On a vu au §1 que l'on peut estimer le terme $g(x_0)$ par un estimateur à noyau. Il nous reste donc à estimer $r(x_0)$. 
<br>
L'estimateur de Nadaraya-Watson de $r(x_0)$ est l'estimateur définit par la formule suivante.
<br>
$\hat{r}_{n,h}(x_0)=\frac{n^{-1}\sum_{i=1}^{n}K_h(x_0-X_i)Y_i}{n^{-1}\sum_{i=1}^{n}K_h(x_0-X_i)} =  \sum_{i=1}^{n}w_{i,n}(x_0)Y_i$ avec $w_{i,n}(x_0)=\frac{K_h(x_0-X_i)}{\sum_{i=1}^{n}K_h(x_0-X_i)}$
<br>
Pour l'estimateur par polynômes locaux, l'approximation localement constante est remplacée par un polynôme.
...
La validation croisée dans le cas de la régression se décompose comme suit :
<br>
On définit, pour i=1,2,...,n l'estimateur $\hat{r}_{(-i),n,h}(x_0)=\sum_{j=1}^{n}Y_jw_{j,n,h,(-i)}$  avec $w_{j,n,h,(-i)}(x_0)=\frac{w_{j,n,h}(x_0)}{\sum_{k\ne i}w_{k,n,h}(x_0)}$ si $j \ne i$ et 0 sinon.
<br>
On définit le score de validation leave-one-out $\hat{R}(h) = \frac{1}{n} \sum_{i=1}^{n}(Y_i - \hat{r}_{(-i),n,h}(X_i))^2$
<br>
On choisi $\hat{h}$ qui minimise la fonction de score, $\hat{h} = min_h\hat{R}(h)$.
<br>
Pour la validation croisé, dans le cas de la régression on utilise la fonction codée en R vue en cours CVbwt (h_CV1) et bw.cv.grid (h_CV2).
<br>
...
legend("top", legend = c( paste("h (CV 2) = ",round(h_CVb,5)),
                         ...
                         paste("h Silverman = ", round(h_silver,5))), lwd = 2, col = c("blue","green","gold",2))
```
On remarque que cet estimateurest plus régulier que Madaraya-Watson implémenté par ksmooth.
<div style="page-break-after: always;"></div>

## Estimation de r en regressant Y1 sur log(X)
...
Et ensuite on implémente avec le paramètre h calculé l'estimateur de Nataraya-Watson avec la fonction de R ksmooth et l'estimateur par polynômes locaux avec la fonctionde R locpoly. 
...
```{r echo=FALSE, fig.height=11, fig.width=10, warning=FALSE}
par(mfrow=c(2,1))
...
# Etude de la densité $\mu$ des  $\xi_i$

## A partir du jeu de donnees Data1

### Distribution approximative de $\tilde{\xi_i}$
...
$$ \text{On a la représentation suivante } Y_i - r(X_i) = \sigma\xi_i \text{ (cas homoscédastique } \sigma  \text{ est constant)}$$
$$ \text{Par définition } \tilde{\xi_i} = Y_i - \hat{r}_{n,h}^{(-)}(X_i) \text{ où } \hat{r}_{n,h}^{(-)}(x) \text{ est un estimateur de r(x).} $$
$$ \text{La distribution approximative de } \tilde{\xi_i} \text{ est celle de } \xi_i \text{ à la constante multiplicative près }  \sigma. $$

### Représentation de la densité $\mu(x)$  $\xi_i$
...
#lines(nu_hdpik$x,dgamma(nu_hdpik$x+0.50,2,3.7),type="l",col="green")
...
### Interêt d'avoir decoupé le jeu de donnees selon *J+* et *J-*
On a ainsi un jeux de données d'apprentissage et de test.
...
### La densité $\mu$	peut-elle être gaussienne

On peut déjà remarquer qu'à la vue des graphiques des estimateurs de la densité  $\tilde{\xi_i}$, on a trop de dissymétrie pour avoir une gaussienne.
On va le préciser avec le protocole empirique de verification ci-après que l'on va appliquer aux données $\hat{r}_{n,h}^{(-)}(X_i)-Y_i=\tilde{\xi_i}$

* test du QQPlot

```{r echo=FALSE, fig.height=5, fig.width=10, warning=FALSE}
par(mfrow=c(1,2))
#dgamma(nu_hdpik$x+0.50,2,3.7)

# qqplot(rgamma(nu_hdpik$x+0.50,2,3.7), X,cex=0.5, 
#          xlab="Quantile estimateur de g(X)",
#          ylab="Quantile théorique - Uniforme U=1/10", #ylim=ylim,
#          main = expression("Q-Q plot pour la loi Gamma"~{}[2,3.7]))
# qqline(rgamma(nu_hdpik$x+0.50,2,3.7),datax=TRUE,col="red")
...
C'est moins sûre si l'on regarde le sous ensemble qui correspond au quantile 85% qui vaut 0.337 (2ème graphique).
...
## A partir du jeu de donnees Data2
  
On cherche à estimer \( \mu \) et $\sigma^2$. Pour cela, on coupe a nouveau l'echantillon en deux et on considère à nouveau  $\tilde{\xi_i}$
...
### Justifier qu'en regressant $\xi_i$ sur $X_i$ on obtient un estimateur de $\sigma^2$

$$ \text{Par définition } \tilde{\xi_i} = Y_i - \hat{r}(X_i) \text{ où } \hat{r}(x) \text{ est un estimateur de r(x).} $$
<br>
$$ \text{Ainsi } \tilde{\xi^2_i} = (Y_i - \hat{r}(X_i))^2  $$

$$ \text{En remplaçant par l'expression de } Y_i = r(X_i) + \sigma(X_i)  \xi_i \text{  on obtient } $$
$$ \tilde{\xi^2_i} = (r(X_i) + \sigma(X_i)  \xi_i - \hat{r}(X_i))^2  $$

$$ \text{Puis en développant } $$
$$ \tilde{\xi^2_i} = (r(X_i) - \hat{r}(X_i))^2 + \sigma(X_i)^2  \xi_i^2  + 2(r(X_i) -\hat{r}(X_i)) \sigma(X_i) \xi_i $$

$$ \text{On conditionne par rapport à } X_i  \text{ et on utilise l'hypothèse d'indépendance de } \xi_i \text{ }$$
$$ E(\tilde{\xi^2_i} \mid X_i) = E((r(X_i) - \hat{r}(X_i))^2 \mid X_i) + E(\sigma(X_i)^2  \mid X_i)E(\xi_i^2)  + 2E((r(X_i) -\hat{r}(X_i)) \sigma(X_i))\mid X_i) E(\xi_i)  $$

$$ \text{Par hypothèse } E(\xi_i)=0 \text{ et } E(\xi_i^2)=1 \text{ on a donc finalement: }$$
$$ E(\tilde{\xi^2_i} \mid X_i) = E((r(X_i) - \hat{r}(X_i))^2 \mid X_i) + E(\sigma(X_i)^2  \mid X_i) $$

$$ \text{Ce qui donne } E(\tilde{\xi^2_i} \mid X_i) = (r(X_i) - \hat{r}(X_i))^2 + \sigma(X_i)^2 $$

$$ \text{Comme } \hat{r}(X_i) \text{ est un estimateur de }  r(X_i) \text{ on a bien le résultat.}  $$
<div style="page-break-after: always;"></div>  
...
Avant d'appliquer les tests classiques (QQPlot, Test de Shapiro et KS) on va regarder comment se comportent les estimateurs à noyaux gaussiens. On estime la densité $\mu$ des $\xi_i$ à partir de l'estimation  $(Y_i-\hat{r}(X_i))/\hat{\sigma}^2(X_i)$ où $\hat{\sigma}^2(X_i)$ est l'estimateur de ${\sigma}^2(X_i)$ obtenu précédement. 
...
On a gagné en symétrie et sur la forme générale par rapport au cas précédent étudié au §3.1.4.
Maintenant on va vérifier l'hypothèse de normalité des données $(Y_i-\hat{r}(X_i))/\hat{\sigma}^2(X_i)$ avec un QQPlot et des tests.
...
D'après ces derniers tests la densité $\mu$ ne peut être gaussienne.
