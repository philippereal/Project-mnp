...
```{r er, eval=FALSE, fig.height=4, include=FALSE}
...
### Représentation sur un même graphe de la regression de Y sur X et Log(X)
...
h_ucv <- bw.ucv(estNu)
h_sil <- 1.06*sqrt(var(estNu))*n**(-1/5)
h_dpik <- dpik(estNu)

dEstNu<-density(estNu)
h_density<-dEstNu$bw

print(paste0("h_dpik : ",h_dpik," h_silver : ", h_sil," h_density ; ",h_density," h_ucv ; ",h_ucv))
...
h_test<-0.06
nu_hucv<-bkde(estNu, kernel = "normal", bandwidth=h_ucv, truncate = TRUE)
nu_hsil<-bkde(estNu, kernel = "normal", bandwidth=h_sil, truncate = TRUE)
nu_hdpik<-bkde(estNu, kernel = "normal", bandwidth=h_dpik, truncate = TRUE)
nu_hdensity<-bkde(estNu, kernel = "normal", bandwidth=h_density, truncate = TRUE)
...
legend("top", legend = c( paste("h (CV) = ",round(h_ucv,5)),
                          paste("h dpik = ", round(h_dpik,5)),
                          paste("h Silverman = ", round(h_sil,5))), lwd = 2, col = c("black","red","gold",2))
```
...
```{r fig.height=7, fig.width=10, include=FALSE}
  ...
  fit2 <- locpoly(param2X, param2Y, bandwidth = h, degre=2, gridsize=1000)
  estNu2 <- param2Y-fit2$y

  fit22 <- locpoly(param2X, estNu2, bandwidth = h,  degre=2,gridsize=1000)
  #dpih Select a Histogram Bin Width
  h32 <- dpih(fit22$y)
...
mu_hDpill<-bkde(estNu2,bandwidth=h_dpill,gridsize = size, kernel = "normal",canonical=FALSE, truncate = TRUE)
...
```{r fig.height=7, fig.width=10, echo=FALSE}
plot(mu_ucv2$x,mu_ucv2$y,type="l",col="red" ,xlab="grid",ylab="Densité", main = expression("Densité estimée pour la loi "*mu*" et différentes fenêtres: h"))
lines(mu_hDpill$x,mu_hDpill$y,type="l",col="blue")
...
kernel_hDpill <- locpoly((mu_hDpill$y)**2, param2X, degre=2, bandwidth = h_dpill,gridsize=200)
