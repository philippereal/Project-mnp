...
Donc plus de précision dans l'intervalle [0+dela , 5] La précision diminue à droite...
...
```{r plot211_d1, echo=FALSE, fig.height=7, fig.width=10}
...
```{r plot212_d1, echo=FALSE, fig.height=7, fig.width=10}
...
#### Estimation de &tilde;r avec Nadaraya-Watson avec la librairie stat - fonction : ksmooth 
...
# Plot data
#plot(Xo, Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Nadaraya–Watson avec ksmooth et différents h")
plot(log(Xo), Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Estimateur construit avec Nadaraya-Watson: ksmooth et différents h")
...
#### Estimateur de &tilde;r(x)  par polynômes locaux de degrès 2
...
# Plot data
#plot(Xo, Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Nadaraya–Watson avec ksmooth et différents h")
plot(log(Xo), Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Estimateur construit par polynômes locaux de degrès 2: locpoly et différents h")
rug(log(Xo), side = 1); rug(Yo, side = 2)
...
### Représentation sur un même graphe de &circ;r(x) et &tilde;r(x) 
```{r echo=FALSE, fig.height=10, fig.width=10}
...
#xlim <- c(0,6)
#ylim <- c(0,1)
plot(xlim, ylim, xlab="X",ylab="Y", log = "",  main="Estimateur de r(x) et ~r(x) par Nadaraya–Watson et locPoly")
...
choix de h établi a la question 2.2: h_dpill=`r h_dpill `
...
lines(nu_hucv$x,nu_hucv$y,type="l",col="black")
...
legend("top", legend = c( paste("h (CV) = ",round(h_ucv2,5)),
                          ...
                          paste("h Silverman = ", round(h_sil2,5))), lwd = 2, col = c("black","red","gold",2))
```
...
On peut utiliser le jeux de données d'apprentissage pour estimer et construire nos modèles, le jeux de test pour calculer une erreur de prédiction.
A partir de cette erreur de prédiction on a un critère pour choisir le meilleur modèle.  
...
qqnorm(nu_hdpik$y, datax=TRUE,  main = expression("Q-Q plot pour la loi "*mu*""),ylim=ylim)
qqline(nu_hdpik$y,datax=TRUE,col=4)
...
```{r echo=FALSE}
 ks.test(nu_hdpik$y,"pnorm",mean(nu_hdpik$y),sd(nu_hdpik$y))
```
...
On peut tracer un graphe des résidus.

<div style="page-break-after: always;"></div>  
  
### 3.2 A partir du jeu de donnees Data2
  
On cherche a estimer \( \mu \)(x) et $\sigma^2(x)$. Pour cela, on coupe a nouveau l'echantillon en deux et on considere a nouveau  \(\xi\)i:
...
#### 3.2.1. Justifier qu'en regressant \( \xi\)i sur $X_i$ on obtient un estimateur de $\sigma^2(x)$

$\begin{align} \text{Par définition } \tilde{\xi_i} = Y_i - \hat{r}(X_i) && \text{où } \hat{r}(x) \text{ est un estimateur de r(x).} \end{align}$
$\begin{align} \tilde{\xi^2_i} = (Y_i - \hat{r}(X_i))^2  \end{align}$
...
A partir de la fenêtre h etablie à la question 2.2: h=`r h_dpill ` on calcul un h optimal pour estimer la densité de: $\tilde{\xi_i} = Y_i - \hat{r}(X_i)$ 
```{r warning=FALSE, include=FALSE}
#on part du h obtenu en 2.2
h<-h_dpill
#h<-0.2186849
...
h_dpih <- dpih(estNu2)
nu_grid2 <- seq(min(estNu2)-h_dpih, max(estNu2)+h_dpih, by=h_dpih)

#validation croisée
h_ucv2=bw.ucv(estNu2)

# Regle Silverman (voir cours)
h_sil2=1.06*sqrt(var(estNu2))*n**(-1/5)
...
mu_dpik<-bkde(estNu2,bandwidth=h_dpik2,gridsize = size, kernel = "normal",canonical=FALSE, truncate = TRUE)
#mu_hcv<-bkde(estNu2,  bandwidth=hCV, gridsize = size, kernel = "normal",canonical=FALSE, truncate = TRUE)
```

On ne conserve que le h obtenu par la fonction dpik (quasimment identique à celui obtenu par CV) et la règle de Silverman 
```{r warning=FALSE, echo=FALSE}
print(paste0("h_dpik : ",h_dpik2, " h_silver : ", h_sil2 , " h_ucv : ", h_ucv2))
```

Puis à  partir de la fonction bkde une estimation de la densité de : $\tilde{\xi_i} = Y_i - \hat{r}(X_i)$
Que l'on trace en fonction des différentes fenêtres obtenues précédemment.
...
lines(mu_dpik$x,mu_dpik$y,type="l",col="blue")

legend("right", legend = c( paste("h dpik = ", round(h_dpik2,5)),
                         #paste("h (CV 1) = ", round(h_CVopt,5)),
                          paste("h Silverman = ", round(h_sil2,5))), lwd = 2, col = c("blue","red"))
```

A partir de la fonction de densité estimée on va construire un estimateur de $sigma(X_i)^2$ 
On calcul par polynômes locaux de degès 2: locpoly la régression de de $\xi^2$ sur $X_i$
...
# Calcul global
s2hat=mean((Y2m[order(X2m)]-Ye$y)**2)# Il faut réordonner Y, car par défaut ksmooth renvoie des valeurs de x.points ordonnées

# amélioration, en prenant en compte l'hétéroscedasticité
# On estime s^2 en regressant le carré des résidus : (Y[order(X)]-Ye$y)**2 sur les X
h_pill_2=dpill(X2m,(Y2m[order(X2m)]-Ye$y)**2)
s2=ksmooth(sort(X2m),(Y2m[order(X2m)]-Ye$y)**2,bandwidth=h_pill_2,x.points=X2m)

plot(s2$x,s2$y,type="l",col="red") # visualisation de l'estimateur de la variance

print(paste0("Err prév initiale : ",var((Y2m[order(X2m)]-Ye$y)**2)," Err prév cas Het : ", mean(((Y2m[order(X2m)]-Ye$y)**2-s2$y)**2)))
```
```{r echo=FALSE, fig.height=7, fig.width=10}

h_pill_1=dpill(param2X,param2Y)
Yestim=ksmooth(param2X,param2Y,bandwidth=h_pill_1,x.points=X2m)

# Calcul global
s2hat=mean((param2Y-Yestim$y)**2)# Il faut réordonner Y, car par défaut ksmooth renvoie des valeurs de x.points ordonnées

# amélioration, en prenant en compte l'hétéroscedasticité
# On estime s^2 en regressant le carré des résidus : (Y[order(X)]-Ye$y)**2 sur les X
h_pill_2=dpill(param2X,(param2Y-Yestim$y)**2)
s2=ksmooth(param2X,(param2Y-Yestim$y)**2,bandwidth=h_pill_2,x.points=param2X)

plot(s2$x,s2$y,type="l",col="red") # visualisation de l'estimateur de la variance
#lines(s2$x,(s(s2$x))**2,col="blue")# superposition de la vraie courbe

print(paste0("Err prév initiale : ",var((param2Y-Yestim$y)**2)," Err prév cas Het : ", mean(((param2Y-Yestim$y)**2-s2$y)**2)))
```
...
h_dpillSigma2 <- dpill(param2X,(mu_dpik$y)**2,gridsize=500)
kernel_hDpill <- locpoly(param2X,(mu_dpik$y)**2, degre=2, bandwidth = h_dpillSigma2, gridsize=500)

plot(kernel_hDpill$x,kernel_hDpill$y,type="l",col="red", xlab="grid",ylab="Densité", 
...
#legend("right", legend = c( paste("h dpik = ", round(h_dpik2,5)), paste("h Silverman = ", round(h_sil2,5))), lwd = 2, col = c("blue","red"))
#legend("right", legend = c( paste("h dpik = ", round(h_dpik2,5)), paste("h Silverman = ", round(h_sil2,5))), lwd = 2, col = c("blue","red"))
print(paste0("Err prév initiale : ",var((Y2m[order(X2m)]-kernel_hDpill$y)**2)," Err prév cas Het : ", mean(((Y2m[order(X2m)]-kernel_hDpill$y)**2-s2$y)**2)))
...
Variance importnate au voisinage de 0 et faible après.  
...
```{r plotA2, echo=FALSE, fig.height=7, fig.width=10}
  qqnorm(mu_sil2$y, datax=TRUE,col=1, main=expression("Q-Q plot de normalité pour la loi "*mu*""))
  qqline(mu_sil2$y,datax=TRUE,col=2)
```
