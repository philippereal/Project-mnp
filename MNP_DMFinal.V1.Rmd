...
```{r plot033_d1, echo=FALSE, fig.height=7, fig.width=10, warning=FALSE}
  ...
  #lines(res008$x,res008$y,type="l",col="brown4",lty=2)
  ...
  legend(3,1.2,c("h=0.03","h=0.08","h=0.2", "h=0.3","h=0.4","h=0.7", "h=1"),
         col=c("gold", "brown4","coral1",2,3,4,5),
         lty=c(1,2,5,6,8,9))
```
...
Si on regarde le critère de l'erreur quadratique un h adapté serait compris entre 0.4 et 0.5
Par la suite on va utiliser d'autres critères pour définir un h optimal.


#### Raffinement pour h: 0.4 - 0.5
```{r include=FALSE}
  res42 <- bkde(x=X, bandwidth=0.42, gridsize= size, kernel = "normal",canonical=FALSE, truncate = TRUE)
  err42 <- mean((res42$y-X)**2)
  
  res43 <- bkde(x=X, bandwidth=0.43, gridsize= size, kernel = "normal",canonical=FALSE, truncate = TRUE)
  err43 <- mean((res43$y-X)**2)
  
  res44 <- bkde(x=X,bandwidth=0.44, gridsize= size, kernel = "normal",canonical=FALSE, truncate = TRUE)
  err44 <- mean((res44$y-X)**2)
  
  res45 <- bkde(x=X,bandwidth=0.45, gridsize= size, kernel = "normal",canonical=FALSE, truncate = TRUE)
  err45 <- mean((res45$y-X)**2)
```
...
```{r plot03_d1, echo=FALSE, fig.height=7, fig.width=10}
  #hist(X, breaks=bins,freq = FALSE )
  ...
  lines(res42$x,res42$y,type="l",col=2)
  lines(res43$x,res43$y,type="l",col=3)
  lines(res44$x,res44$y,type="l",col=4)
  lines(res45$x,res45$y,type="l",col=5)
...
En appliquant la règle de Siverman, on obtient le h suivant:
```{r }
n<-length(X)
hh<-1.06*sqrt(var(X))*n**(-1/5)
hh
```
...
```{r fig.height=5, fig.width=8, echo=FALSE}
  resH<-bkde(X, kernel = "normal", bandwidth=h, truncate = TRUE)
  resHH<-bkde(X, kernel = "normal", bandwidth=hh, truncate = TRUE)
  
  plot(resH$x,resH$y,type="l",col="blue",  main ="Densité par noyaux pour h obtenues par différentes méthodes.", xlab="X", ylab="Y")
  lines(resHH$x,resHH$y,type="l",col="red")
...
lines(density(x = X, bw = 0.5, kernel = "epanechnikov"), col = 2)
lines(density(x = X, bw = 0.5, kernel = "gaussian"), col = 3)
lines(density(x = X, bw = 0.5, kernel = "rectangular"), col = 4)
lines(density(x = X, bw = 0.5, kernel = "triangular"), col = 5)
lines(density(x = X, bw = 0.5, kernel = "biweight"), col = 6)
lines(density(x = X, bw = 0.5, kernel = "cosine"), col = 7)
lines(density(x = X, bw = 0.5, kernel = "optcosine"), col = 8)
...
```{r echo=FALSE, fig.height=7, fig.width=10, warning=FALSE}
  h_ucv <-bw.ucv.mod(x = X, plot.cv = TRUE)
  h_ucv
...
dpik           |  `r hdpik `
density        |  0.4294
ESM            | entre 0.4 et 0.5
Regle Silverman|  `r hh `


```{r echo=FALSE, fig.height=8, fig.width=12}

resd <- bkde(x=X,bandwidth=0.4294,gridsize=size)
estim_dpik <- bkde(x=X, kernel = "normal",bandwidth=hdpik, truncate = TRUE)
  
plot(resH$x,resH$y,type="l",col=1, lty=1, main ="Estimation de la densité par noyaux pour différents h", xlab="X", ylab="Y" )
rug(X, side = 1)
lines(estim_dpik$x,estim_dpik$y,type="l",col=2, lty=2)
lines(resd$x,resd$y,type="l",col=3, lty=5)
lines(res040$x,res040$y,type="l",col=4, lty=6)
lines(res44$x,res44$y,type="l",col=5, lty=8)
lines(resHH$x,resHH$y,type="l",col=6, lty=9)

legend(6,0.8,c("bw.ucv", "dpik","density", "h=0.4","h=0.45", "h=Silverman"),
         col=c(1,2,3,4,5,6),
         lty=c(1,2,5,6,8,9))
```
...
```{r plot3_QQ, echo=FALSE}
  ...
  #qplot(sample = runif(200, min=0, max=10), data = XFrame)
...
```{r plot2314_d1, include=FALSE,eval=FALSE}
...
##### Find optimum CV bandwidth, with sensible grid
```{r, echo=FALSE, fig.height=7, fig.width=10, eval=FALSE}
# Bandwidth
hCV <- bw.cv.grid(X = Xo, Y = Yo, plot.cv = TRUE)
hCV
#lines(grid, mNW(x = grid, X = X, Y = Y, h = h), col = 2)
#legend("top", legend = c("Nadaraya-Watson"), lwd = 2, col = 2)
```
...
#### Estimateur de Nadaraya-Watson avec la librairie stat - fonction : ksmooth - h =  `r hDpill ` et h =  `r hCV `
...
ksmooth <- ksmooth(Xo, Yo, "normal", bandwidth = h)
ksmoothCV <- ksmooth(Xo, Yo, "normal", bandwidth = hCV)
...
lines(ksmooth, col = 2)
lines(ksmoothCV, col = "blue")

legend("top", legend = c("Nadaraya-Watson h Dpill = 0.2186849","Nadaraya-Watson h (CV) = 0.1200601"), lwd = 2, col = c(2,"blue"))
```

L'estimateur est sensible au choix de h.
L'estimateur de Nadaraya-Watson est très oscillant par construction.

#### Estimateur de Nadaraya-Watson à partir de la fonction recodé NW - h =  `r hDpill ` et h =  `r hCV 
```{r echo=FALSE, fig.height=7, fig.width=10}
# Bandwidth
h <- hDpill
#hCV<-0.1200601
...
# Plot data
plot(Xo, Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Nadaraya–Watson avec la fonction mNW et différents h")
rug(X, side = 1); rug(Y, side = 2)
lines(xGrid, mNW(x = xGrid, X = Xo, Y = Yo, h = h), col = 2)
lines(xGrid, mNW(x = xGrid, X = Xo, Y = Yo, h = hCV), col = "blue")
legend("top", legend = c("Nadaraya-Watson h Dpill = 0.2186849","Nadaraya-Watson h (CV) = 0.1200601"), lwd = 2, col = c(2,"blue"))
```


#### Estimateur par polynômes locaux
Utilisation de la fonction locpoly du package Kersmooth avec h =  `r hDpill `
Attention à ordonner l'échantilon (en X) pour garder la cohérence des couples (X,Y), car sinon KernSmooth va réordonner les X sans tenir compte de la composante Y.  
...
lp0 <- KernSmooth::locpoly(x = Xo, y = Yo, bandwidth = h, degree = 0, range.x = c(-10, 10), gridsize = gdsize)
lp1 <- KernSmooth::locpoly(x = Xo, y = Yo, bandwidth = h, degree = 1, range.x = c(-10, 10), gridsize = gdsize)
lp2 <- KernSmooth::locpoly(x = Xo, y = Yo, bandwidth = h, degree = 2, range.x = c(-10, 10), gridsize = gdsize)
...
lo0 <- loess(Yo ~ Xo, degree = 0, span = span)
lo1 <- loess(Yo ~ Xo, degree = 1, span = span)
...
# Prediction at x = 2
x <- 2
pred2<-lp1$y[which.min(abs(lp1$x - x))] # Prediction by KernSmooth::locpoly
## [1] 0.6271391

pred<-predict(lo1, newdata = data.frame(X = x)) # Prediction by loess

# Plot data
plot(Xo, Yo,pch=20,cex=0.01,xlab="X",ylab="Y", main ="Regression par polynômes locaux avec locpoly")
rug(X, side = 1); rug(Y, side = 2)
lines(lp2$x, lp2$y, col = 3, lty=1, pch = 12,lwd = 1)
lines(lp0$x, lp0$y, col = "blue", lty=1,pch = 12,lwd = 1)
lines(lp1$x, lp1$y, col = "red", lty=1, pch = 12, lwd = 1)
#lines(xGrid, predict(lo0, newdata = data.frame(X = xGrid)), col = "cyan1", lty = 2)

 legend("top", legend = c("Constant","Linéaire","Degrés 2"), lwd = 2, col = c("red","blue",3), lty = c(1,1,1))
      
```


```{r plot23_d1, echo=FALSE}
 
  #Y_CVopt=locpoly(X,Y,drv=0,degree=2,kernel="normal",bandwidth=0.173141)
  #Y_CVglobal=locpoly(X,Y,drv=0,degree=2,kernel="normal",bandwidth=hglobal)
  #plot(X,Y,pch=20,cex=0.01,xlab="X",ylab="Y", main="polynome local pour hglobal")
  
  #lines(Y_CVopt$x,r(Y_CVopt$x),col="red")  
  #lines(Y_CVopt,lty="dashed",col="red")
  #lines(Y_CVglobal,lty="dashed",col="cyan1")
  
  # Le h local ne fonctionne pas bien lorsqu'un bloc contient trop peu d'observations; essayer la
  # méthode alternative qu'il faut coder.
```


```{r plot2401_d1, echo=FALSE}

#### Utilisation directe de dpill et locpoly
#dpill Select a Bandwidth for Local Linear Regression
#locpoly Estimate Functions Using Local Polynomials

#comment because take time
#hz <- dpill(X, Y)

#fitz <- locpoly(X, Y, bandwidth = hz)
#plot(X,Y,pch=20,cex=0.01,xlab="X",ylab="Y",main="Utilisation directe de dpill et locpoly")
#lines(fitz, col="cyan1")
...
#### Estimation de la régression de Log(X) avec Nadaraya-Watson avec la librairie stat - fonction : ksmooth - h =  `r ln_hDpill ` et h =  `r 0.4046381 `
```{r echo=TRUE, fig.height=7, fig.width=10, warning=FALSE}
...
log_ksmoothCV <- ksmooth(log(Xo), Yo, "normal", bandwidth = ln_hCV)
...
lines(log_ksmoothDpill, col = "red")
lines(log_ksmoothCV, col = "blue")

legend("top", legend = c("Nadaraya-Watson h Dpill = 0.649558","Nadaraya-Watson h (CV) = 0.4046381"), lwd = 2, col = c("red","blue"))

```

#### Estimateur de la régression de Log(X) avec Nadaraya-Watson à partir de la fonction recodé NW - h =  `r ln_hDpill ` et h =  `r ln_hCV `
```{r echo=TRUE, fig.height=7, fig.width=10, warning=FALSE}
d1o <- order(d1[,"X"])
Xo <- d1[d1o,"X"]
Yo <- d1[d1o,"Y1"]
...
plot(log(Xo),Y,pch=20,cex=0.01,xlab="X",ylab="Y",main="Estimation de la regression de Log(X) avec Nadaraya–Watson: fonction NW")
rug(log(Xo), side = 1); rug(Yo, side = 2)

lines(xGrid, mNW(x = xGrid, X = log(Xo), Y = Yo, h = ln_hDpill), col = "red")
lines(xGrid, mNW(x = xGrid, X = log(Xo), Y = Yo, h = ln_hCV), col = "blue")
...
```{r echo=TRUE, fig.height=7, fig.width=10, warning=FALSE}
plot(log(Xo),Yo,pch=20,cex=0.01,xlab="X",ylab="Y",main="Estimation de la regression de Log(X) par locpoly")
rug(log(Xo), side = 1); rug(Yo, side = 2)

lines(locpoly(log(Xo), Yo, degree = 2, bandwidth = ln_hDpill), col="red")
lines(locpoly(log(Xo), Yo, degree = 2, bandwidth = ln_hCV), col="blue")

legend("top", legend = c("h=0.649558","h=0.4046381"), lwd = 2, col = c("red","blue"), lty = 1)
```
...
NW = mNW(x = grid, X = Xo, Y = Yo, h = hDpill)
log_NW = mNW(x = grid, X = log(Xo), Y = Yo, h = ln_hDpill)

locpoly <- locpoly(Xo, Yo, bandwidth = hDpill)
log_locpoly <- locpoly(log(Xo), Yo, bandwidth = ln_hDpill)

ksmoothDpill <- ksmooth(Xo, Yo, "normal", bandwidth = hDpill)
log_ksmoothDpill <- ksmooth(log(Xo), Yo, "normal", bandwidth = ln_hDpill)
...
#h<-0.2186849
n<-length(x = Xo)
EstimR <- locpoly(paramX, paramY, degre=2, bandwidth = h,gridsize=1000)
...
```{r plot3.1.1.4, eval=FALSE, fig.height=7, fig.width=10}
 # hist(locpoly_hDpill$y, breaks=grid_dpih,freq = FALSE,xlab="grid",ylab="Densité", main="Histogramme et densité" )
 # 
 # lines(r_ucv$x,r_ucv$y,type="l",col="blue")
 # lines(r_sil$x,r_sil$y,type="l",col="red")
 # 
 # legend(6,3,c("h=0.003554934", "h=0.02757643"), col=c("blue", "red"),lty=1:1)
```
...
On peut utilser le jeux de données d'apprentissage pour estimer notre lois, et le jeux de test
