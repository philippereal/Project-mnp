...
- Si h est trop petit (courbes marron du graphique ci-dessus) l'estiamteur est très oscillant, la variance est importante mais le biais est faible.
...
xlim <- c(-0.2,6)
ylim <- c(0,1.2)
plot(xlim, ylim, xlab="X",ylab="Densité g(x) estimée", log = "",  main="Estimation de la densité par noyaux pour différents h")
#rug(X, side = 1)
...
```{r plot3_QQ, echo=FALSE, fig.height=4, fig.width=10}
  #ylim<-c(0,2)
  qqplot(runif(200, min=0, max=10), Xo,cex=0.5, ylab="Quantile - g(X)",xlab="Quantile - Uniforme U=1/10", #ylim=ylim,
         main = expression("Q-Q plot pour la loi Uniforme"~{}[U == 1/10]))
  qqline(Xo,datax=TRUE,col="red")
...
```
A la vue du graphique QQPlot par rapport à la loi uniforme (U=1/10) l'hypothèse selon laquelle g est uniforme n'est pas si déraisonnable.
En particulier pour les $X_i \in [0+,2]$ qui représente une forte concentration des données environ 63%. Mias celà ne semble pas suffisant pour valider l'hypothèse.

### 1.4 Zone de l'espace où l'estimation de r sera plus précise
On aura plus de précision où la densité des données est importante (variance constante, ne dépend pas de X).
...
#### Estimateur de $\tilde{r}(x)$ par polynômes locaux de degrès 2
...
#xlim <- c(0,6)
#ylim <- c(0,1)
plot(xlim, ylim, xlab="X",ylab="Y", log = "",  main="Estimateur par Nadaraya–Watson et locPoly de degrès 2")
...
On remarque que les estimateurs basés sur la régression de $Y_i$ $log(X_i)$ sont bien plus réguliers, voire linéaires.  
...
hist(estNu, breaks=nu_grid,freq = FALSE,xlab="grid",ylab="Densité",main = expression("Histogramme et Estimation de la densité "*mu*" de "*xi*" pour différentes fenêtres: h"))
rug(estNu, side = 1); 
...
Ce qui nous ammène à penser que l'hypthèse d'homoscédasticité est bien vérifiée. 
...
Peu de variance où la densité est élevé dans l'intervalle [0+,1]. Les points sont ressérés.
Ensuite la vraince augmente en même temps que la densité des points diminue.
...
#### 3.2.2. La densitée \(\mu\) peut-elle être gaussienne

A partir de l'estimateur $\hat{r}(X_i)$ on va construire un estimateur de $sigma(X_i)^2$ 
Pour celà on construit un estimateur en regressant sur $X_i$ le carré des résisus: $(Y_i-\hat{r}(X_i))^2$
On estime la densité $\mu(x)$ des $\xi_i$  partir de l'estimation  $(Y_i-\hat{r}(X_i))/\hat{\sigma}^2(X_i)$ où $\hat{\sigma}^2(X_i)$ est l'estimateur de ${\sigma}^2(X_i)$ obtenu
...
hist(estMu_ks, breaks=mu_grid,freq = FALSE,xlab="X - grid",ylab=expression("Y - Estimateur de "*mu*""), xlim=xlim, ylim=ylim,
     main = expression("Histogramme et Estimation de la densité "*mu*" de "*xi*" pour différentes fenêtres: h  - cas hétérostatique"))
rug(estMu_ks, side = 1); 
...
A la vue des graphiques la densité $\mu(x)$ ne semble pas être gaussienne.
...
- 1- test du QQPlot
```{r echo=FALSE, fig.height=7, fig.width=10}
  qqnorm(mu_hdpik$y, datax=TRUE,col=1, main=expression("Q-Q plot de normalité pour la loi "*mu*""))
  qqline(mu_hdpik$y,datax=TRUE,col=2)
```

L'hypothèse de normalité est rejettée.
Ou bien gaussien sur un intervalle très faible, autour de 0: ]0+,0.02[
...
p-value faible l'hypothèse de normalité est rejettée.
