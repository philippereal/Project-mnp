...
<div style="page-break-after: always;"></div>
## 3. Etude de la densitée \(\mu\) des  $\xi_i$
...
la p-value est faible l'hypothèse de normalité est rejetée.
...
On cherche a estimer \( \mu \) et $\sigma^2$. Pour cela, on coupe a nouveau l'echantillon en deux et on considère a nouveau  \(\xi\)iy
...
plot(param2X,param2Y,xlab="X",ylab="Y2", main = "Jeux de données Data2 observations X en abscisse et Y en ordonnée.")
```
...
On estime la densité $\mu(x)$ des $\xi_i$ à partir de l'estimation  $(Y_i-\hat{r}(X_i))/\hat{\sigma}^2(X_i)$ où $\hat{\sigma}^2(X_i)$ est l'estimateur de ${\sigma}^2(X_i)$ obtenu précédement.
...
hist(estMu_ks, breaks=mu_grid,freq = FALSE,xlab="X - grid",ylab=expression("Y - Estimateur de "*mu*""), xlim=xlim, ylim=ylim,
     main = expression("Histogramme et Estimateurs de la densité "*mu*" de "*xi*" pour différentes fenêtres: h  - cas hétérostatique"))
rug(estMu_ks, side = 1); 
...
plot(mu_hdpik$x,mu_hdpik$y,type="l",col="blue",xlim=xlim, ylim=ylim,xlab="grid",ylab="Densité",
     main = expression("Estimateurs de la densité "*mu*" de "*xi*" pour différentes fenêtres: h  - cas hétérostatique"))
rug(estMu_ks, side = 1); 
...
On a cependant gagné en symétrie par rapport au cas précédent étudié au §3.1.4
...
```
Le test de Shapiro-Wilk donne une de p-value < 2.2e-16. 
L'hypothèse de normalité est rejetée.
