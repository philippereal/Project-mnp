...
La méthode par noyau est une généralisation de la méthode d'estimation par histogramme(fréquence des observations dans une fenêtre).
Elle permet de construire un estimateur avec de meilleur propriété de régularité notamment de continuité.
$\text{On a un échantillon } (X_1,X_2,...X_N) \text{ i.i.d, réalisation d'une variable aléatoire X dont on cherche à estimer la densité g}$
$\text{Un estimateur par noyaux est donné par } \hat{g}(x)= \frac{1}{Nh}\sum_{n=1}^{N} K(\frac{x_0-X_i}{h} )$
$\text{K est le noyau régularisant que l'on prendra ici gaussien } K(x) = \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}x^2}$

$\text{L'estimateur est basé sur la série d'approximation suivante } g(x_0) \displaystyle \approx_{h \to 0}   E(\frac{1}{h} K(\frac{x_0-X}{h} ))\approx_{n \to \infty} \frac{1}{nh}\sum_{i=1}^{n} K(\frac{x_0-X_i}{h} ) := \hat{g}_{n,h}(x_0)$ 
...
On utilise la fonction de R stat_smooth de la librairie ggplot2 avec l'option:
- method=lm  pour construire la régression linéaire simple (en rouge). 
- method=loess  pour construire la régression de type loess (en bleure). 
...
La localisation est assurée par la fonction de poids le noyau K. On le prendra gaussien $K(x) = \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}x^2}$
